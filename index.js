export function EasypeersSignal(opts) {
  ...
  async function gossipToClosePeers(message, gossipId, fraction, excludePeers = [], relayingPeerId, from) {
    ...
    for (let peerId of closestPeers) {
      ...
      if (dataChannel.readyState === "open") {
        ...
        if (opts && opts.debug) console.debug(`Data channel with ${peerId} is not open. Current state: ${dataChannel.readyState}`)
      }
    }
  }
  ...
  async function createPeerConnection(targetId) {
    ...
    rtcPeerConnection.ondatachannel = event => {
      ...
      dataChannel.onopen = event => {
          if (opts && opts.debug) console.debug("Data channel is open with " + targetId)
      ...
      dataChannel.onmessage = async event => {
        ...
        if (seenGossipIds[message.gossipId]){
          if (opts && opts.debug) console.debug(`Already seen message with gossipId ${message.gossipId}`)
          return
        }
        ...
        gossipToClosePeers(JSON.stringify(message), message.gossipId, 0.33, excludePeers, peerId, message.from)
      }      
    }
  ...
  function startWebSocketConnection() {
    ws = new WebSocket( opts && opts.wsServer ? opts.wsServer : 'ws://localhost:8080')
  
    ws.onopen = async () => {
      if (opts && opts.debug) console.debug('Connected to the signaling server')
    ...
    ws.onmessage = msg => {
      ...
      switch(data.type) {
        case 'server-is-alive':
          if (!hasHandledServerIsAlive && reconnectAttempts > 0 && peerCount == 0) {
            let data = {
              type: "hello",
              id: peerId,
              reconnected: reconnectAttempts > 0
    ...
    ws.onclose = event => {
      ...
      if (opts && opts.debug) console.debug("WebSocket is closed. Reconnect will be attempted in " + currentReconnectDelay / 1000 + " second.", event.reason)
      hasHandledServerIsAlive = true
    }
  }
